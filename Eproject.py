...
def processing(img) :
	...
	# 그린 채널만 사용하여 히스토그램평활화를 위한 clahe 알고리즘 적용
	img = clahe.apply(green)
	tmp = img

	# 침식과 팽창연산을 이용한 3번의 열림연산과 닫힘연산 수행
	tmp_img = cv2.morphologyEx(img, cv2.MORPH_OPEN,
cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(7,7)), iterations = 1)
	tmp_img = cv2.morphologyEx(tmp_img, cv2.MORPH_CLOSE,
cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(7,7)), iterations = 1)
	tmp_img = cv2.morphologyEx(tmp_img, cv2.MORPH_OPEN,
cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(13,13)), iterations = 1)
	tmp_img = cv2.morphologyEx(tmp_img, cv2.MORPH_CLOSE,
cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(13,13)), iterations = 1)
	tmp_img = cv2.morphologyEx(tmp_img, cv2.MORPH_OPEN,
...
cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(25,25)), iterations = 1)
	...
	image, contours, hierarchy = cv2.findContours(threshold.copy(),cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE) 
        # 노이즈 제거를위한 Contours 적용
	mask = np.ones(img.shape[:2], dtype="uint8") * 255 # 필터마스크
	for cnt in contours:
		if cv2.contourArea(cnt) <= 200: 
		# 영역이 200보다 작으면 mask적용
 			cv2.drawContours(mask, [cnt], -1, 0, -1)
	image = cv2.bitwise_and(img, img, mask=mask) # mask 적용
	ret,threshold = cv2.threshold(image,15,255,cv2.THRESH_BINARY_INV) 
	# threshold
	normal = cv2.erode(threshold, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(3,3)),
iterations=1) # 침식연산
...
# In[172]:
x_data = processing("../10003_left.jpeg") 
# 학습데이터가 아닌 임의의 데이터가 저장된 경로
